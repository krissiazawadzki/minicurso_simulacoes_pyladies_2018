#!/usr/bin/env python
"""
Modified by Vinicius and Krissia for the PyLadies-Sanca
minicourse on simulations. Added some default configurations
and included colors to the planets.

Fixed value of acceleration ( a = F/m, it was only a = F)
...
# Used to select override parameters
MODE = 0
...
def initializeConstants(mode=0):
    '''
    Overrides the initial values
    '''
    global PLANETS
    ...
    if mode<=0:
        PLANETS = 5
        g_listOfPlanets = [
            Planet(State(50,0,0,55), color=(255,50,50)),
            Planet(State(100,0,0,36), color=(55,255,50)),
            Planet(State(200,0,0,28), color=(55,55,250)),
            Planet(State(350,0,0,22), color=(250,0,0)),
            Planet(State(800,0,0,14), color=(250,100,50),mass=0.1),
            ]

    if mode==1:
        PLANETS = 1
        g_listOfPlanets.append(
                Planet(State(150, 300, 0, 2), color=(50,50,255))
                )
    ...
    if mode >= 2:
        ...
        for i in range(0, PLANETS):
            g_listOfPlanets.append(
                    Planet(getRandomState(), color=getRandomColor() )
                    )

    for planet in g_listOfPlanets:
        planet._st._x += WIDTHD2
        planet._st._y += HEIGHTD2
    # And God said: Let there be lights in the firmament of the heavens...
    return g_listOfPlanets
...
def getRandomState():
    return State(
               float(random.randint(-WIDTHD2, WIDTHD2)),
               float(random.randint(-HEIGHTD2, HEIGHTD2)),
               14*(float(random.randint(0, 300)/100.)-1.5),
               14*(float(random.randint(0, 300)/100.)-1.5))
...
class Planet:
    ...
    def __init__(self, state = State(0,0,0,0), color=(255,255,255), radius=1.5, mass=None):
        self._st = state
        self._r = radius
        if mass == None:
...
def main():
    ...
    global g_listOfPlanets, MODE
    if len(sys.argv) == 2:
        MODE = int(sys.argv[1])

    g_listOfPlanets = initializeConstants(MODE)
    ...
    while True:
        ...
        for p in g_listOfPlanets:
            if not p._merged:  # for planets that have not been merged, draw a
                ...
                pygame.draw.circle(win, p._color,
